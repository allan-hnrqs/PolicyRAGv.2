...
class ToolAgentBudget:
    ...
    max_rerank_chunks: int = 12
    max_indexed_chunks: int = 16
...
def _normalize_action(parsed: dict[str, object], *, budget: ToolAgentBudget | None = None) -> dict[str, object]:
    resolved_budget = budget or ToolAgentBudget()
    action = str(parsed.get("action", "")).strip()
    if action not in {"search_inventory", "fetch_page", "rerank_fetched_chunks", "retrieve_indexed_chunks", "answer"}:
        raise ValueError(f"Unsupported tool agent action: {action}")
    normalized: dict[str, object] = {"action": action}
    if action in {"search_inventory", "rerank_fetched_chunks", "retrieve_indexed_chunks"}:
        ...
        if action == "search_inventory":
            normalized["top_k"] = min(top_k, resolved_budget.max_search_results)
        elif action == "rerank_fetched_chunks":
            normalized["top_k"] = min(top_k, resolved_budget.max_rerank_chunks)
        else:
            normalized["top_k"] = min(top_k, resolved_budget.max_indexed_chunks)
...
class ToolUsingOfficialSiteAgentRunner:
    def __init__(
        ...
        *,
        ...
        indexed_tool_profile_name: str | None = None,
        max_steps: int = 5,
        max_live_pages: int = 6,
        max_live_chunks: int = 24,
    ) -> None:
        ...
        self.indexed_tool_profile = load_profile(indexed_tool_profile_name, settings) if indexed_tool_profile_name else None
        ...
        self.max_steps = resolved_budget.max_steps
        self.max_live_pages = resolved_budget.max_live_pages
        self.max_live_chunks = resolved_budget.max_live_chunks
        self.max_search_results = resolved_budget.max_search_results
        self.max_rerank_chunks = resolved_budget.max_rerank_chunks
        self.max_indexed_chunks = resolved_budget.max_indexed_chunks
    ...
    def _plan_next_action(
        ...
        *,
        ...
        return _normalize_action(parsed, budget=self.budget)
...
def build_tool_agent_run_manifest(
    ...
    *,
    ...
    return {
        ...
        "mode": "tool_using_official_site_agent_v2",
        ...
        "max_search_results": resolved_budget.max_search_results,
        "max_rerank_chunks": resolved_budget.max_rerank_chunks,
        "max_indexed_chunks": resolved_budget.max_indexed_chunks,
        "indexed_tool_profile_name": indexed_tool_profile_name,
    }
            ...
            if steps:
                ...
                for step in steps:
                    ...
                    lines.append(
                        ...
                        f"origin={step.get('evidence_origin')}"
                    )
        lines.append("")
    return "\n".join(lines).rstrip() + "\n"
